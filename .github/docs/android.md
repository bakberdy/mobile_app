...
> **Where to run the commands:** Steps **A1–A4** are written for **repo root** (paths are relative to the project: `android/...`, `config/run/...`, `build/...`).

## Step A1 — Generate the upload keystore (one per flavor)

> Run from: **repo root**.

Each flavor gets its **own** upload keystore (mirror of iOS's per-flavor provisioning profiles). Generate once; keep the `.jks` under the repo at `android/keystores/...` (the file is **gitignored** and never committed). The commands below match `storeFile` in `key.properties.example`. `keytool` will prompt for store password, key password (press Enter to reuse the store password), and a Distinguished Name.
...
keytool -genkey -v \
  -keystore android/keystores/production/upload.jks \
...
keytool -genkey -v \
  -keystore android/keystores/development/upload.jks \
  -keyalg RSA -keysize 2048 -validity 10000 \
  -alias upload
```
...
```bash
keytool -list -v -keystore android/keystores/production/upload.jks -storepass '<store password>'
```
...
| Key             | Meaning                                                                                                                                                                                                                                                                     |
| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `storeFile`     | Path to the upload keystore (`.jks`) — **relative to the `android/` directory** (e.g. `keystores/production/upload.jks` → `android/keystores/production/upload.jks`). Do **not** use a path starting with a single `/` at the real disk root (e.g. `/keys/...`) by mistake. |
...
fastlane supply \
  ...
  --track internal \
  --json_key android/keys/play-service-account.json \
...
> **Treat this file like a password.** Anyone with it can publish to your Play apps. Keep it at `android/keys/play-service-account.json` (gitignored under `android/keys/`), `chmod 600`, never commit it.
...
5. From **repo root**, save the key in the project as `android/keys/play-service-account.json` (copy the download there with Finder or your editor). Then:

```bash
mkdir -p android/keys
chmod 600 android/keys/play-service-account.json
```
...
| Used by                         | How                                                                                                                                                                          |
| ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Local Fastlane (Step A4 Path 2) | `--json_key android/keys/play-service-account.json` (from repo root) or `keys/play-service-account.json` in `PLAY_STORE_SERVICE_ACCOUNT_JSON_PATH` under `android/keys/.env` |
...
base64 -i android/keystores/production/upload.jks | tr -d '\n' | pbcopy

# Development keystore → secret ANDROID_KEYSTORE_BASE64_DEVELOPMENT
base64 -i android/keystores/development/upload.jks | tr -d '\n' | pbcopy
```

On Linux: `base64 -w0 android/keystores/production/upload.jks | xclip -selection clipboard` (or `wl-copy`).
...
- **Tag releases:** `.github/workflows/release-on-tag.yml` triggers on `release: published`, validates the tag must be `development-MAJOR.MINOR.PATCH` or `production-MAJOR.MINOR.PATCH`, bumps `pubspec.yaml`, then calls this workflow with `flavor: <prefix>` and `track: internal`. The bumped commit's SHA is passed as `ref` so the AAB is built from the version-bumped source, not the original tag commit. If the **GitHub Release** has a **description** (the release body), the first **500** characters (plain text) are sent as **en-US** “What’s new” for this build on Google Play; iOS in the same run uses a separate artifact and TestFlight.
